...
> **원본 CSV 는 저장소에 포함되지 않습니다** (`train.csv` · `test.csv` 각 55MB, `.gitignore` 처리).
> 고객명 · 계좌번호 · IP · MAC 이 그대로 들어 있어 공개 저장소에 올리지 않습니다.
>
> ✅ **전처리를 마친 3차 세트는 커밋합니다** — `X_tr` · `y_tr` · `X_va` · `y_va.parquet` (합계 8.4MB).
> 라벨 인코딩된 수치 58열뿐이라 식별 정보가 없습니다. 이걸 빼 두면 clone 한 사람과
> 배포본에서 대시보드 첫 화면의 KPI·분포 차트가 통째로 비어 버립니다.
> `구`·`구구` 세대와 `.pq` 는 계속 제외합니다.
>
> 아래는 스키마와 재생성 방법입니다.
