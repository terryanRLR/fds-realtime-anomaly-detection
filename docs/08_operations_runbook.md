...
| `FDS_DEMO_MODE` | (미설정) | 🎬 시연용 시각 축소. 아래 §2 참고 |
| `FDS_DEMO_WINDOW_H` | `6` | 시연 창 길이(시간). `FDS_DEMO_MODE=1` 일 때만 의미 있음 |
...
> 런처의 바인딩 기본값은 `127.0.0.1` 입니다(`fds_config.bat` 의 `BIND_ADDR`).
> LAN 공유가 필요할 때만 `0.0.0.0` 으로 바꾸고, 끝나면 되돌리세요.
> ngrok 은 `127.0.0.1:포트` 로 붙으므로 외부 공개에는 바꿀 필요가 없습니다.

### 🎬 시연 모드 — 남의 눈으로 화면을 볼 때

포트폴리오·면접에서 관제 화면을 보여줄 때 쓰는 **표시 전용** 옵션입니다.

```bash
set FDS_DEMO_MODE=1          # 켜기 (기본은 꺼짐)
set FDS_DEMO_WINDOW_H=6      # 창 길이, 기본 6시간
run_ops_dashboard.bat
```

**왜 필요한가** — 로컬 원장에 남은 미판정 이력이 오래돼서, 화면 상단에
`최장 대기 47일 6시간` · `SLA 30분 초과 113건` 이 뜹니다. 화면은 **정확하게**
동작하고 있지만, 처음 보는 사람에게는 "운영이 방치됐다" 로 읽힙니다.

**무엇을 하는가** — DB 를 고치지 않고, 조회 결과의 **시각만** 최근 구간으로
비례 축소합니다.

| | 시연 모드 OFF | ON (6시간) |
|---|---|---|
| 최장 대기 | `47일 6시간` | **`5시간 58분`** |
| 미판정 건수 | 113건 | 113건 (동일) |
| 큐 순서 | — | 동일 |
| 상대 위치 | — | 보존 (오차 3e-05) |

건수·순서·상대 위치는 실제 데이터 그대로이고 **절대 간격만** 축소됩니다.
켜면 화면 맨 위에 축소 사실과 원래 구간(`2026-06-26 ~ 2026-08-10, 44.8일`)을
적은 배지가 뜹니다 — 재기준된 시각을 실제 운영 시각으로 오인하지 않도록
숨기지 않습니다.

**적용 범위 밖** (의도적)

- **발송 이력** 탭은 실제 과거 시각을 보여줍니다. 이 조회는 `since_hours` 를
  SQL 에서 실제 시각으로 거르기 때문에, 표시값만 밀면 필터가 0행을 반환해
  앞뒤가 맞지 않습니다.
- **워처 상태**도 그대로입니다. 이건 과거 기록이 아니라 살아 있는 프로세스의
  현재 상태라서, 최근 시각으로 덮으면 죽은 프로세스를 살아 있다고 보여주는
  것이 됩니다. 워처 패널까지 보여주려면 워처를 실제로 켜세요.

구현은 [`pipeline/demo_mode.py`](../pipeline/demo_mode.py) 한 파일이고,
연결 지점은 `ops_queries.alert_queue()` 한 곳입니다(알림 행이 나오는 유일한
깔때기라 트리아지·헤더 배지·교대 요약·실시간 피드가 같은 기준을 봅니다).
플래그가 꺼져 있으면 아무 일도 하지 않습니다 — 자가검증 12종은 켠 상태·끈
상태 모두 통과합니다.
