...
title FDS Team Server - ops_dashboard.py only

set "LOGFILE=%~dp0start_ops_team_log.txt"
echo ============================================== > "%LOGFILE%"
echo  start_ops_team.bat - %date% %time% >> "%LOGFILE%"
echo ============================================== >> "%LOGFILE%"

:: --------------------------------------------------
:: [EDIT HERE] 이 런처만의 동작 스위치. 공용 경로/포트는 fds_config.bat 에서.
:: LLAMA_ENABLE=0 이 기본값: ops_dashboard.py 는 아직 LLM 기능이 없음.
:: 나중에 세션5 기능(AI 분석/챗봇)을 이식하면 이걸 1로 바꾸면 됨.
:: --------------------------------------------------
set LLAMA_ENABLE=0
...
echo.
echo  ================================================
echo    FDS Team Server - ops_dashboard.py only
echo  ================================================
echo.
if "%LLAMA_ENABLE%"=="0" (
    echo   NOTE: ops_dashboard.py doesn't use the LLM yet, so llama.cpp
    echo   is skipped by default. Set LLAMA_ENABLE=1 above once session5
    echo   features are ported into ops_dashboard.py.
    echo.
)
...
set "HELP_ST=%~dp0_run_ops_streamlit.cmd"
...
>>"%HELP_ST%" echo title ops_dashboard.py (Streamlit)
...
:: ---------- [3/6] Start llama.cpp (optional) ----------
:: NOTE: the retry loop below uses goto/labels, which must NOT be nested
:: inside a parenthesized if-block (cmd.exe's parser breaks on that), so
:: the on/off switch is done with "if not ... goto llama_skip" instead.
if not "%LLAMA_ENABLE%"=="1" goto llama_skip
...
:llama_skip
echo  [3/6] Skipped (LLAMA_ENABLE=0^).
...
:: ---------- [5/6] Start Streamlit (ops_dashboard.py) and wait ----------
echo  [5/6] Starting ops_dashboard.py on port %OPS_PORT%...

netstat -ano | findstr :%OPS_PORT% | findstr LISTENING >nul
if not errorlevel 1 (
    echo   [ERROR] Port %OPS_PORT% is already in use - is ops_dashboard.py
    echo   [ERROR] already running from another launcher? Details:
    netstat -ano | findstr :%OPS_PORT% | findstr LISTENING
    echo   [ERROR] Close that process, or change OPS_PORT in fds_config.bat.
    echo.
    echo  Press any key to close this window...
    pause >nul
    exit /b 1
...
start "ops_dashboard.py" cmd /k "%HELP_ST%"
...
:st_timeout
echo        [ERROR] ops_dashboard.py is NOT answering on port %OPS_PORT%.
echo        [ERROR] Check the "ops_dashboard.py" window. ngrok will NOT start.
echo.
echo  Press any key to close this window...
pause >nul
exit /b 1
...
echo  [6/6] Starting ngrok tunnel to ops_dashboard.py (port %OPS_PORT%)...
start "ngrok tunnel" cmd /k "ngrok http 127.0.0.1:%OPS_PORT%"
...
echo   ----------------------------------------------------
echo   CHECK: the target above MUST end with :%OPS_PORT%

:summary
echo.
echo  ================================================
echo   ops_dashboard.py team server is up.
echo  ================================================
echo   Local access:  http://localhost:%OPS_PORT%
if "%WATCHER_ENABLE%"=="1" echo   Watcher:       watching %DASHBOARD_DIR%\inbox
echo   Log file: %LOGFILE%
echo   REMINDER: this must run on the SAME PC as the watcher, since
echo   ops_dashboard.py reads fds_results.db and watcher.log locally.
echo  ================================================
echo.
echo  Press any key to close this window...
pause >nul
